...
- **Nothing here is off-limits to edit.** There is no CODEOWNERS file and no per-path
  restriction. What stops an unreviewed change is the same thing that stops any other:
  every pull request to `main` needs a human approval, and nobody bypasses it.
- **Call it out explicitly in the PR description.** Name the file and say what the
  change permits that was not permitted before. This is the whole safeguard: since no
  rule forces a particular reviewer onto these paths, a rule change riding along in a
  feature diff is genuinely easy to miss. Do not bundle it with unrelated work.
